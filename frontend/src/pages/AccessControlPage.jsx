...
export default function AccessControlPage() {
  ...
  // Wallet access requests state
  const [walletRequests, setWalletRequests] = useState([]);
  const [actionMsg, setActionMsg] = useState(null);
  const [busyAddr, setBusyAddr] = useState(null);

  const refreshWalletRequests = () => {
    setWalletRequests(getPendingAccessRequests());
  };
  ...
  async function handleGrant(address) {
    setBusyAddr(address);
    setActionMsg(null);
    try {
      const res = await grantAccessWallet(address);
      setActionMsg({
        type: "info",
        text: `Granted access to ${address} ${res.txHash ? `(Tx: ${res.txHash.slice(0, 10)}...)` : ""}`,
      });
      refreshWalletRequests();
    } catch (e) {
      setActionMsg({ type: "alert", text: e.message });
    } finally {
      setBusyAddr(null);
    }
  }
  ...
  async function handleReject(address) {
    ...
    setActionMsg(null);
    try {
      const res = await rejectAccessWallet(address);
      setActionMsg({
        type: "alert",
        text: `Rejected access for ${address} ${res.txHash ? `(Tx: ${res.txHash.slice(0, 10)}...)` : ""}`,
      });
      refreshWalletRequests();
    } catch (e) {
      setActionMsg({ type: "alert", text: e.message });
  ...
  return (
    <>
      <div className="page-header-row">
        <div>
          <h1 className="page-title">Access Control & Wallet Permissions</h1>
          <p className="page-subtitle">
            Super Admin wallet authorization portal & fine-grained role/resource permission evaluation on AccessControl.sol.
          </p>
        </div>
      </div>

      {actionMsg && (
        <div className={`banner ${actionMsg.type}`} style={{ marginBottom: 16 }}>
          <span>{actionMsg.text}</span>
          <button className="action" onClick={() => setActionMsg(null)} style={{ padding: "4px 8px", fontSize: 11 }}>
            Dismiss
          </button>
        </div>
      )}

      {/* Super Admin Wallet Access Manager Panel */}
      <Panel title="MetaMask Access Requests Manager" badge={`${walletRequests.length} Requests`}>
        {walletRequests.length === 0 ? (
          <EmptyState>No wallet access requests recorded yet. Visitors can request access from the login screen.</EmptyState>
        ) : (
            ...
            <table>
              <thead>
                <tr>
                  ...
                  <th>Status</th>
                  <th>Smart Contract Authorization</th>
                </tr>
              </thead>
              <tbody>
                {walletRequests.map((req) => (
                  <tr key={req.address}>
                    <td>
                      <code style={{ color: "#38bdf8", background: "rgba(56, 189, 248, 0.1)", padding: "2px 8px", borderRadius: "4px" }}>
                        {req.address}
                      </code>
                    </td>
                    <td className="muted">{req.timestampStr || new Date(req.requestedAt).toLocaleString()}</td>
                    <td>
                      <Badge tone={req.status === "APPROVED" ? "allow" : req.status === "REJECTED" ? "deny" : "warn"}>
                        {req.status}
                      </Badge>
                    </td>
                    <td>
                      <div style={{ display: "flex", gap: "8px" }}>
                        <button
                          ...
                          disabled={busyAddr === req.address || req.status === "APPROVED"}
                          onClick={() => handleGrant(req.address)}
                          style={{ padding: "5px 12px", fontSize: "12px" }}
                        >
                          {busyAddr === req.address ? "Processing..." : req.status === "APPROVED" ? "Granted ✓" : "Grant Access"}
                        </button>
                        <button
                          className="action btn-danger"
                          disabled={busyAddr === req.address || req.status === "REJECTED"}
                          onClick={() => handleReject(req.address)}
                          style={{ padding: "5px 12px", fontSize: "12px" }}
                        >
                          {busyAddr === req.address ? "Processing..." : req.status === "REJECTED" ? "Rejected ✗" : "Reject"}
                        </button>
                      </div>
                    </td>
                  </tr>
