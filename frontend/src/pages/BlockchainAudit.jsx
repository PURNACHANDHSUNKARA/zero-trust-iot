...
export default function BlockchainAudit() {
  ...
  async function load() {
    ...
    try {
      ...
      let trail = [];
      try {
        trail = await getAuditTrailOnChain(100);
      } catch (err) {
        console.warn("On-chain fetch error, trying snapshot fallback:", err.message);
      }

      if (!trail || trail.length === 0) {
        // Fallback to static snapshot authentication logs
        const res = await fetch("/data/authentication_logs.json");
        if (res.ok) {
          const raw = await res.json();
          trail = raw.map((log) => ({
            deviceId: log.device_id || "IOT001",
            resource: log.resource || "Telemetry",
            action: log.action || "READ",
            decision: log.status === "SUCCESS" ? "ALLOW" : "DENY",
  ...
  function exportCSV() {
    ...
    const headers = ["Device ID", "Resource", "Action", "Decision", "Timestamp"];
      ...
      <Panel
        ...
        action={
          <div style={{ display: "flex", gap: "8px" }}>
            {events.length > 0 && (
              <button className="btn-secondary-white" onClick={exportCSV} style={{ fontSize: "12px", padding: "6px 12px" }}>
                📥 Export CSV
              </button>
            )}
